...
app.post('/api/batch-update', async (req, res) => {
  try {
    const { pageIds, title, sections, seo } = req.body;
    if (!pageIds?.length) return res.status(400).json({ success: false, error: 'pageIds array required' });
    ...
    for (const pid of pageIds) {
      try {
        // Fetch current content
        const page = await wpFetch(`/wp/v2/pages/${pid}?context=edit&_fields=id,title,content`, {}, req);
        let content = page.content?.raw || '';
        ...
        for (let i = 0; i < 3; i++) {
          if (sections?.[i]?.imageUrl && imgUrls[i]) {
            content = content.replace(imgUrls[i], sections[i].imageUrl);
            if (sections[i]?.imageId) {
              content = content.replace(/wp-image-\d+/, `wp-image-${sections[i].imageId}`);
              content = content.replace(/"id":\d+/, `"id":${sections[i].imageId}`);
            }
          }
        }
        ...
        const updateBody = { content };
        if (title) updateBody.title = title;
        await wpFetch(`/wp/v2/pages/${pid}`, { method: 'POST', body: JSON.stringify(updateBody) }, req);
        ...
        if (seo && (seo.focusKeyphrase || seo.metaDescription || seo.seoTitle || seo.additionalKeyphrases?.length)) {
          ...
          await wpFetch('/aioseo/v1/post', {
            method: 'POST', _creds: getCreds(req),
            body: JSON.stringify({
              id: pid, postId: String(pid), post_type: 'page', postType: 'page',
              ...(seo.seoTitle ? { title: `#post_title #separator_sa ${seo.seoTitle}` } : {}),
              ...(seo.metaDescription ? { description: seo.metaDescription } : {}),
              ...(Object.keys(kp).length ? { keyphrases: kp } : {}),
            }),
